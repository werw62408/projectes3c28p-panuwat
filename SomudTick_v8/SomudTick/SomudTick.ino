...
void drawAppTitle(const String& title, const String& right = "");
...
void rewriteToday() {
  File f = LittleFS.open(logPath(curDay), "w");
  if (!f) return;
  for (auto& e : todayEv) f.print(evLine(e));
  f.close();
}
void recomputeLast() {
  lastSeen.clear();
...
void defaultActs() {
  ...
  add("meal", "Meal", 0xA9822B, "", 1, 3, 1, 0);
  add("snack", "Snack", 0xB5586F, "", 1, 2, 2, 0);
...
void logEvent(int i, float v) {
  ...
  todayEv.push_back(e);
  File f = LittleFS.open(logPath(curDay), "a");
  if (f) { f.print(evLine(e)); f.close(); }
  lastSeen[e.id] = e.t;
  ...
  if (allTime) {
    ...
    if (root) {
      ...
      while (f) {
        while (f.available()) {
          String l = f.readStringUntil('\n'); Ev e;
          if (parseLine(l, e)) { auto it = idx.find(e.id); if (it != idx.end()) { st[it->second].total += e.v; st[it->second].totalCnt++; } }
        }
        f.close();
        f = root.openNextFile();
      }
...
void drawHeader() {
  ...
  String ds = timeApprox ? String("time not set") : String(EN_DOW[tm.tm_wday]) + " " + tm.tm_mday + " " + EN_MON[tm.tm_mon];
  int room = hdrChipX0 - 6 - left;
  if (room > 20) txt(FS, fitText(FS, ds, room), left, HDR_H / 2, HDRSOFT, D_ML);
}
int tabOf(Screen s) {
  switch (s) { case S_HOME: case S_KEYPAD: return 0; case S_STATS: return 1; case S_SET: case S_WIFI: case S_BT: case S_KBD: return 3; default: return 2; }
}
// tab widths follow the word length, so "Settings" fits on the narrow screen
const char* FTR_TABS[] = {"Log", "Stats", "Apps", "Settings"};
void footerTabs(int* x0) {   // x0[0..4] = tab edges
  spr.setFont(FS);
  int tot = 0, tw[4];
  for (int i = 0; i < 4; i++) { tw[i] = spr.textWidth(FTR_TABS[i]); tot += tw[i]; }
  int pad = (W - tot) / 4;
  x0[0] = 0;
  for (int i = 0; i < 4; i++) x0[i + 1] = x0[i] + tw[i] + pad;
  x0[4] = W;
}
void drawFooter() {
  int x0[5]; footerTabs(x0);
  int cur = tabOf(scr);
...
void drawAppTitle(const String& title, const String& right) {
  ...
  txt(FB, fitText(FB, title, W - 8 - 78 - (rw ? rw : 66)), 78, HDR_H + 17, INK, D_ML);   // no right text: leave room for a button
}
...
void drawHome() {
  ...
  // today line: how many goals are done, one dot per activity that has a goal
  {
    ...
    String t = goals ? String("Goals done ") + done + " / " + goals : String("Today");
    txt(FB, t, 8, y + SUM_H / 2 - 2, INK, D_ML);
    spr.setFont(FB); int dx = 8 + spr.textWidth(t) + 10;
    for (auto& a : acts) {
      if (!(a.goal > 0 && a.goalType) || dx > W - 12) continue;
  ...
  for (size_t i = 0; i < acts.size(); i++) {
    ...
    if (suf.length()) txt(FS, fitText(FS, suf, tw - bw - 20), x + 13 + bw, y + 34, fg2);
    ...
    // time since last
    uint32_t last = lastSeen.count(a.id) ? lastSeen[a.id] : 0;
    txt(FS, fitText(FS, (od ? "! " : "") + agoStr(last), tw - 16), x + 9, y + 56, od ? fg : fg2);
    // buttons: [-] undo, [+] add
...
String gapStr(double sec) {
  long m = (long)(sec / 60);
  return (m >= 60 ? String(m / 60) + " h " : String("")) + String(m % 60) + " min";
}
...
void drawStatTabs() {
  int w = (W - 12 - 8) / 3;
  for (int i = 0; i < 3; i++) btn(6 + i * (w + 4), HDR_H + 4, w, 26, STAT_VIEWS[i], statView == i);
}
...
void drawStatDays() {
  ...
  txt(FS, "Avg gap " + (s.gapN ? gapStr(s.gapSum / s.gapN) : String("-")), 10, infoY, SOFT);
  uint32_t last = lastSeen.count(a.id) ? lastSeen[a.id] : 0;
  txt(FS, last ? "Last " + hhmm(last) : String(""), W - 10, infoY, SOFT, D_TR);
}
void drawStatHours() {
  statsNeed(heatDays);
  int top = statTop();
...
void statsTap(int x, int y) {
  if (y < HDR_H + 32) {   // view buttons
    int w = (W - 12 - 8) / 3, v = (x - 6) / (w + 4);
...
void settingsUI(bool draw, int tx, int ty) {
  ...
  if (setPage == 0) {
    ...
    line("Version", "SomudTick v8", false);
  }
...
const char* APP_SUBS[N_APPS] = {"Photos, clips", "Air con", "4 games", "Weather"};
...
void render() {
  ...
  if (scr == S_KEYPAD) drawKeypadFooter(); else if (scr != S_SUDOKU && scr != S_KBD) drawFooter();   // Sudoku uses the whole screen
  spr.pushSprite(0, 0);
  dirty = false;
}
...
void onTap(int x, int y) {
  ...
  if (y < HDR_H) {
    if (x >= hdrChipX0 - 4 && x <= hdrChipX1 + 4) { place = place == 'H' ? 'U' : 'H'; prefs.putChar("place", place); dirty = true; }
    return;
  }
  ...
  if (scr == S_KEYPAD) {
    if (y >= FTR_Y) {
      if (x < W / 2) { scr = S_HOME; }
      else { float v = kpVal.toFloat(); if (v > 0) logEvent(kpAct, v); scr = S_HOME; }
      dirty = true; return;
    }
    KpGeo g = kpGeo();
    if (x >= g.x0 && y >= g.y0) {
  ...
  if (y >= FTR_Y) {   // bottom tabs
    int x0[5]; footerTabs(x0);
    int t = 3; for (int i = 0; i < 4; i++) if (x < x0[i + 1]) { t = i; break; }
    const Screen TAB_SCR[4] = {S_HOME, S_STATS, S_APPS, S_SET};
    if (t >= 0 && t < 4) goScreen(TAB_SCR[t]);
    return;
  }
  switch (scr) {
    case S_HOME: {
...
void buttonTask() {
  ...
  if (v != prev && millis() - tChg > 40) {
    ...
    if (!v) { bT0 = millis(); longDone = false; }
      ...
      wake();
      if (!wasOff && !acts.empty() && scr != S_GAME && scr != S_SUDOKU) { logDefault(0); refreshStatsIfVisible(); }
    }
  }
...
void apiLog() {
  ...
  if (server.hasArg("v")) { float v = server.arg("v").toFloat(); if (v <= 0) { server.send(400, "text/plain", "bad v"); return; } logEvent(i, v); }
  else logDefault(i);
...
void apiExport() {
  ...
  if (days > 0) {
    ...
    File root = LittleFS.open("/log");
    File f = root.openNextFile();
    while (f) { String nm = f.name(); nm.replace(".csv", ""); keys.push_back(nm); f.close(); f = root.openNextFile(); }
    root.close();
    std::sort(keys.begin(), keys.end());
  }
  for (auto& k : keys) {
    if (!LittleFS.exists(logPath(k))) continue;
    File f = LittleFS.open(logPath(k), "r");
...
void apiUploadData() {
  ...
  if (u.status == UPLOAD_FILE_START) {
    ...
    upFile = SD_MMC.open(joinPath(dir, safeFileName(u.filename)), FILE_WRITE);
...
void setupWeb() {
  server.on("/upload", HTTP_POST, apiUploadDone, apiUploadData);
  server.on("/api/sd", HTTP_GET, apiSdList);
  server.on("/api/sd/del", HTTP_POST, apiSdDel);
  server.on("/api/sd/dirs", HTTP_GET, apiSdDirs);
  server.on("/api/sd/op", HTTP_POST, apiSdOp);
  server.on("/", HTTP_GET, [] { server.send_P(200, "text/html; charset=utf-8", INDEX_HTML); });
  server.on("/api/state", HTTP_GET, apiState);
  server.on("/api/log", HTTP_POST, apiLog);
  server.on("/api/undo", HTTP_POST, apiUndo);
  server.on("/api/acts", HTTP_GET, apiActsGet);
  server.on("/api/acts", HTTP_POST, apiActsPost);
  server.on("/api/stats", HTTP_GET, apiStats);
  server.on("/api/time", HTTP_POST, apiTime);
  server.on("/api/place", HTTP_POST, apiPlace);
  server.on("/api/wifi", HTTP_GET, apiWifiGet);
  server.on("/api/wifi", HTTP_POST, apiWifiPost);
  server.on("/export.csv", HTTP_GET, apiExport);
...
void loop() {
  ...
  if (ms - tick > 1000) {
    ...
    for (auto& a : acts) {
      if (overdue(a) && !remindedFor.count(a.id)) {
        remindedFor[a.id] = 1;
        if (scr == S_SUDOKU) sudokuClose();   // keep the game
        if (scr == S_SAND) sandClose();
        wake(); scr = S_HOME; dirty = true;
        ledFlash(0xFFFFFF, 1500);
      }
    }
  }
  static uint32_t saveT = 0;
  if (ms - slow > 30000) { slow = ms; readBattery(); }
