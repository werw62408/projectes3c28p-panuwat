...
const int SA_BAR = 30;            // tool bar height
...
const int SA_NCOL = 8;
const uint32_t SA_COLS[SA_NCOL] = {0xE8C88A, 0xF08A6C, 0xF4A6C8, 0xB8A6F0, 0x8FE0B8, 0x8CC8F0, 0xF2D060, 0xF2F2F2};
const uint32_t SA_BG = 0x0E1621;
...
// ---------------- drawing ----------------
int saBtnW() { return (W - 60) / 5; }
void saToolRect(int k, int& x, int& w) { w = saBtnW(); x = 58 + k * w; }   // k = tool 0..4
void saDrawTool(int k, int cx, int cy) {
  switch (k) {
    case ST_SAND:
      ...
      spr.fillCircle(cx - 3, cy - 3, 2, C(0xFFFFFF));
      break;
    case ST_WATER:
    ...
    case ST_EMPTY:   // floor with a gap and an arrow going down
      spr.fillRect(cx - 10, cy + 5, 6, 3, C(0xBDBDBD)); spr.fillRect(cx + 4, cy + 5, 6, 3, C(0xBDBDBD));
      spr.fillRect(cx - 1, cy - 8, 3, 9, C(0xBDBDBD));
      spr.fillTriangle(cx - 5, cy, cx + 5, cy, cx, cy + 6, C(0xBDBDBD));
      break;
  }
}
void sandDraw() {
  ...
  if (!saDrain) spr.drawFastHLine(0, top + saGH * SA_CELL, W, C(0x3A4656));   // the floor
  // tool bar
  spr.fillRect(0, 0, W, SA_BAR, C(0x000000));
  spr.fillRoundRect(3, 3, 52, SA_BAR - 6, 8, C(0x26303C));
  spr.fillTriangle(10, SA_BAR / 2, 17, SA_BAR / 2 - 6, 17, SA_BAR / 2 + 6, C(0xFFFFFF));
  txt(FS, "Back", 20, SA_BAR / 2, 0xFFFFFF, D_ML);
  for (int k = 0; k < 5; k++) {
    int x, w; saToolRect(k, x, w);
    bool on = k == ST_EMPTY ? saDrain : k == saTool;
    if (on) { spr.fillRoundRect(x + 2, 3, w - 4, SA_BAR - 6, 8, C(0x33475C)); spr.fillRect(x + w / 2 - 6, SA_BAR - 4, 12, 2, C(0xFFFFFF)); }
    saDrawTool(k, x + w / 2, SA_BAR / 2);
  }
  if (saHint) {
    int bw = W - 24, bh = 150, bx = 12, by = SA_BAR + (H - SA_BAR - bh) / 2;
    spr.fillRoundRect(bx, by, bw, bh, 12, C(0x000000));
    spr.drawRoundRect(bx, by, bw, bh, 12, C(0x8CC8F0));
    auto line = [&](const lgfx::IFont* f, const String& s, int y, uint32_t col) { txt(f, fitText(f, s, bw - 16), W / 2, y, col, D_MC); };
    line(FL, "Sand & Water", by + 22, 0xFFFFFF);
    line(FS, "Drag your finger to pour.", by + 52, 0xFFFFFF);
    line(FS, "Tap Sand again: colour", by + 74, 0xFFFFFF);
    line(FS, "Arrow: empty it all", by + 96, 0xFFFFFF);
    line(FS, saJoy ? "Stick: tilt = wind" : "No score, no rush.", by + 124, 0x9AA7B4);
  }
  spr.pushSprite(0, 0);
}
