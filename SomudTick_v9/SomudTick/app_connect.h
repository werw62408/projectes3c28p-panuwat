...
bool kbdShow = false;
int kbdLayer = 0;                 // 0 abc, 1 ABC, 2 123
...
void drawKbd() {
  ...
  struct { float a, b; const char* t; bool dark; } bot[] = {
    {0, 1.5f, kbdLayer == 1 ? "abc" : "Aa", kbdLayer == 1}, {1.5f, 3, kbdLayer == 2 ? "abc" : "123", kbdLayer == 2},
...
void kbdTap(int x, int y) {
  ...
  int k = kbKeyAt(x, y);
  if (!k) return;
  if (k == 1) kbdLayer = kbdLayer == 1 ? 0 : 1;
  else if (k == 2) kbdLayer = kbdLayer == 2 ? 0 : 2;
...
void drawWifi() {
  ...
  String st, st2;
  if (WiFi.status() == WL_CONNECTED) { st = "Connected: " + WiFi.SSID(); st2 = "Internet OK   IP " + WiFi.localIP().toString(); }
...
void btScan() {
  spr.fillRoundRect(W / 2 - 80, H / 2 - 20, 160, 40, 10, C(INK));
  txt(FB, "Scanning 5 s...", W / 2, H / 2, ONINK, D_MC);
  spr.pushSprite(0, 0);
  ...
  BLEScanResults* r = sc->start(5, false);
  bdevs.clear();
...
void drawBt() {
  ...
  btn(W - 70, HDR_H + 4, 64, 26, "Scan", false);
  int y = HDR_H + 36;
  spr.fillRoundRect(6, y, W - 12, 44, 10, C(CARD));
  txt(FS, fitText(FS, btScanned ? String(bdevs.size()) + " devices nearby" : String("Tap Scan to find devices"), W - 28), 14, y + 6, INK);
