...
bool newsFetch(int cat) {
  ...
  newsC[cat] = got; newsAtC[cat] = millis();
  return true;
}
...
void netLoad(bool force) {
  ...
  if (netTab == 1 && (force || curNews().empty() || millis() - newsAtC[newsCat] > NET_REFRESH_MS)) {
    if (newsCat == 0) oilFetch();   // oil price box (Tech only) is optional; news still shows if it fails
    if (!newsFetch(newsCat)) netMsg = "Could not get the news. Tap Reload to try again.";
  }
  lastTouchMs = millis();
  dirty = true;
}
