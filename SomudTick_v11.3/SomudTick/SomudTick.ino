...
void loop() {
  ...
  if (scr == S_MAZE || scr == S_BLOCKS) {   // the new games draw themselves too
    if (scr == S_MAZE) mazeLoop(); else blocksLoop();
    ledTask(); netPoll(); rtcTask();
    if (scr != S_MAZE && scr != S_BLOCKS) { dirty = true; render(); }
    delay(1);
    return;
  }
