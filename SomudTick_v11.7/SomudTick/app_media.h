...
// About > SD card (v11.5): the space used with decimals, and how many photos and clips are on the card.
// (A 4 GB card with 20 small photos really uses about 1 MB: every file takes at least one 32 KB block.)
String bytesText(uint64_t b) {
  char s[24];
  if (b < 1048576ULL) snprintf(s, sizeof s, "%u KB", (unsigned)((b + 1023) / 1024));
  else if (b < 1073741824ULL) { double m = b / 1048576.0; snprintf(s, sizeof s, m < 100 ? "%.1f MB" : "%.0f MB", m); }
  else snprintf(s, sizeof s, "%.1f GB", b / 1073741824.0);
  return s;
}
int sdPhotos = -1, sdClips = 0; bool sdMany = false;
void sdCountIn(const String& dir, int depth) {
  std::vector<String> kids;   // folders are opened after this one is closed (only a few files can be open at once)
  File d = SD_MMC.open(dir); if (!d) return;
  for (File f = d.openNextFile(); f; f = d.openNextFile()) {
    String n = baseOf(f.path());
    if (n.length() && n[0] != '.' && n != "System Volume Information") {
      if (f.isDirectory()) kids.push_back(joinPath(dir, n));
      else { String e = lowerExt(n); if (isImg(e)) sdPhotos++; else if (isVid(e)) sdClips++; }
    }
    f.close();
    if (sdPhotos + sdClips >= 2000) { sdMany = true; break; }   // a huge card: stop counting (About must open fast)
  }
  d.close();
  if (depth < 6) for (auto& k : kids) { if (sdMany) break; sdCountIn(k, depth + 1); }
}
void sdCountFiles() { sdPhotos = 0; sdClips = 0; sdMany = false; if (sdMount()) sdCountIn("/", 0); else sdPhotos = -1; }
String sdCardText() {
  if (usbDriveOn) return "USB drive on";   // the computer owns the card
  if (!sdOk) return "no card";
  return bytesText(SD_MMC.usedBytes()) + " / " + bytesText(SD_MMC.totalBytes());
}
String sdFilesText() {
  if (sdPhotos < 0) return "-";
  if (sdMany) return "2000+ files";
  return String(sdPhotos) + (sdPhotos == 1 ? " photo, " : " photos, ") + String(sdClips) + (sdClips == 1 ? " clip" : " clips");
}
...
void drawFiles() {
  ...
  for (size_t i = 0; i < fList.size(); i++) {
    ...
    navAdd(6, y, W - 12, FROW - 4);   // press = open, hold = the Rename / Move / Delete menu
    spr.fillRoundRect(6, y, W - 12, FROW - 4, 8, C(CARD));
  ...
  if (fmUi == FU_MENU && fmIdx >= 0 && fmIdx < (int)fList.size()) {
    ...
    navModal(2, H / 2);   // Back = tap outside the window (closes it)
...
// Reminder sound (v11.5): a short, soft "ti-ding" (about 0.3 s). Nothing when the sound is off or a clip is playing.
// The volume is the one in Settings > Sound volume.
uint32_t beepCount = 0;   // how many reminder sounds were asked for (the simulator tests read it)
void remindBeep() {
  if (volIdx < 0 || audRun) return;
  beepCount++;
  if (!audioInit()) return;
  digitalWrite(AUD_EN, LOW);   // amp on
  const int RATE = AUDIO_RATE;
  const float F[2] = {1760.0f, 2349.0f};                            // two notes, the second one higher
  const int LEN[2] = {RATE * 90 / 1000, RATE * 160 / 1000};         // 90 ms, 160 ms
  int16_t b[160];
  for (int t = 0; t < 2; t++) {
    int n = LEN[t];
    for (int i = 0; i < n; i += 160) {
      ...
      for (int k = 0; k < m; k++) {
        int j = i + k;
        float env = min(1.0f, min(j, n - j) / (RATE * 0.008f));   // 8 ms fade in and out: no click
        b[k] = (int16_t)(7000 * env * sinf(2 * PI * F[t] * j / RATE));
      }
      i2s.write((uint8_t*)b, m * 2);
    }
    memset(b, 0, sizeof b);
    // 40 ms gap after the first note. After the last one 120 ms of silence: the sound chip still holds about 90 ms
    // (6 x 240 frames) when write() returns, so the amp must stay on until that has played (v11.7: the end was cut, with a click)
    int quiet = t ? RATE * 120 / 1000 / 160 : RATE * 40 / 1000 / 160;
    for (int i = 0; i < quiet; i++) i2s.write((uint8_t*)b, sizeof b);
  }
  digitalWrite(AUD_EN, HIGH);   // amp off
}
...
bool lcdConfirm(const String& title, const String& line) {
  ...
  lgfx::touch_point_t tp;
  int sel = -1;   // joystick: 0 Cancel, 1 Move (the ring shows after the first push)
  auto ring = [&]() {
    for (int k = 0; k < 2; k++) { int bx = k ? x + 16 + bw : x + 8;
      for (int r = 0; r < 3; r++) lcd.drawRoundRect(bx - 3 + r, by - 3 + r, bw + 6 - 2 * r, 40 - 2 * r, 10, k == sel ? 0xFC60 : TFT_BLACK); }
  };
  while (true) {
    uint32_t t0 = millis();
    while (!lcd.getTouch(&tp)) {
      server.handleClient(); delay(10); if (millis() - t0 > 30000) return false;   // no answer = Cancel
      JoyEv je = navWaitEvent();
      if (je == JE_BACK) return false;
      if (je == JE_LEFT || je == JE_RIGHT) { sel = je == JE_RIGHT ? 1 : 0; ring(); t0 = millis(); }
      if (je == JE_PRESS) { if (sel < 0) { sel = 0; ring(); } else return sel == 1; }
    }
...
void runImageViewer(int idx) {
  ...
  while (idx >= 0 && idx < (int)fList.size()) {
    ...
    bool gone = false;
    int act = -1;   // 0 prev, 1 back, 2 delete, 3 next
    // joystick: left / right = photo before / after, press = Back, hold the press = Delete
    while (!lcd.getTouch(&tp)) {   // left alone: go back to the list, so the screen can dim and turn off as usual
      ...
      JoyEv je = navWaitEvent();
      if (je == JE_LEFT) act = 0; else if (je == JE_RIGHT) act = 3; else if (je == JE_PRESS || je == JE_BACK) act = 1; else if (je == JE_HOLD) act = 2;
      if (act >= 0) break;
    }
    if (gone) break;
    int w = lcd.width();
...
void runVideo(const String& path) {
  ...
  while (!stop) {
    ...
    if (joyOk && joyDown()) { stop = true; while (joyDown()) delay(5); joyNavReset(); }   // press the stick = stop
  }
  if (snd) audioStop();
  f.close();
  free(buf);
...
int videoEndMenu(const String& name) {
  ...
  int sel = -1;   // joystick: up / down chooses, press = do it
  const int RET[3] = {1, 2, 0};
  auto ring = [&]() {
    int w = lcd.width(), bw = min(w - 40, 220), x = (w - bw) / 2;
    for (int k = 0; k < 3; k++) for (int r = 0; r < 3; r++) lcd.drawRoundRect(x - 3 + r, y0 + k * 46 - 3 + r, bw + 6 - 2 * r, 44 - 2 * r, 10, k == sel ? 0xFC60 : TFT_BLACK);
  };
  while (millis() - t0 < 15000) {
    server.handleClient();
    JoyEv je = navWaitEvent();
    if (je == JE_BACK) return 0;
    if (je == JE_UP || je == JE_DOWN) { sel = sel < 0 ? 0 : constrain(sel + (je == JE_DOWN ? 1 : -1), 0, 2); ring(); t0 = millis(); }
    if (je == JE_PRESS) { if (sel < 0) { sel = 0; ring(); } else return RET[sel]; }
    if (lcd.getTouch(&tp)) {
