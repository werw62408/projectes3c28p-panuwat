...
String csvRow(const Ev& e) {   // one line of the CSV export (web page and USB drive)
    ...
    name.replace("\"", "\"\"");
    String unit = i >= 0 ? acts[i].unit : String("");   // in quotes too: a unit like "km,ok" must stay in one column
    unit.replace("\"", "\"\"");
    String row = String(dt) + "," + e.t + "," + e.id + ",\"" + name + "\"," + fmtNum(e.v) + ",\"" +
                 unit + "\"," + (e.place == 'U' ? "มอ" : "บ้าน") + "," + (e.ok ? 1 : 0) + "\n";
    return row;
}
...
void apiExport() {
  int days = constrain((int)server.arg("days").toInt(), 0, 3650);   // 0 = ทั้งหมด (at most 10 years: a huge number ran the board out of memory)
...
void apiUploadData() {
  ...
  if (u.status == UPLOAD_FILE_START) {
    upErr = "";
    if (pinLocked() || !pinOk()) { upErr = "PIN"; return; }   // no PIN (or locked after 5 wrong tries): nothing is written
...
void apiSdList() {
  ...
  if (sdOk) {
    ...
    d["dir"] = dir;
    if (dir == "/" || sdPhotos < 0) sdCountFiles();   // photos and clips on the card (counted again at the top folder), v11.6
    d["photos"] = sdPhotos; d["clips"] = sdClips; d["many"] = sdMany;
...
// v11.4: not on the board's own Wi-Fi (SomudTick): joining it already needs its password, and some phone
// browsers (a page opened from inside another app) forget the cookie, so they asked for the PIN every time.
// On the home Wi-Fi (anyone at home can reach the board) the PIN is still asked.
...
int pinFails = 0; uint32_t pinLockUntil = 0;   // 0 = not locked
// still locked? (counted as "time since", so it also ends right when millis() starts again from 0 after 49.7 days)
bool pinLocked() {
  if (pinLockUntil && (int32_t)(millis() - pinLockUntil) >= 0) pinLockUntil = 0;
  return pinLockUntil != 0;
}
bool viaHotspot() { return apOn && server.client().localIP() == WiFi.softAPIP(); }   // the phone is on the board's own Wi-Fi
bool pinOk() {   // no answer sent (the upload uses this)
  if (viaHotspot()) return true;
...
bool webAuth() {
  if (pinLocked()) { server.send(429, "text/plain", "locked"); return false; }
  if (pinOk()) { pinFails = 0; return true; }
  bool tried = server.header("Cookie").indexOf("stpin=") >= 0 || server.hasArg("pin");
  if (tried && ++pinFails >= 5) { pinFails = 0; pinLockUntil = millis() + 60000; if (!pinLockUntil) pinLockUntil = 1; }
