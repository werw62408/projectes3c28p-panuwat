...
// v11.7: Game Boy games go to /roms as they are (no converting)
async function sendGames(files){
  for(let i=0;i<files.length;i++){const f=files[i];
    if(!/\.gb$/i.test(f.name)){toast(f.name+': ไม่ใช่ไฟล์ .gb');continue}
    if(f.size>4*1048576){toast(f.name+': ใหญ่เกิน 4 MB');continue}
    try{await upload(f,'/roms',safeName(f.name,'game')+'.gb',p=>setStatus(`เกม ${i+1}/${files.length}: กำลังส่ง ${Math.round(p*100)}%`))}catch(e){toast(f.name+': '+e.message)}}
  setStatus('ส่งเกมเสร็จแล้ว เล่นที่บอร์ด: Apps > Games > Game Boy');loadSd();
}
...
// v11.6 (iPhone, Safari): a clip gives pictures only when its player is ON the page, and it may be played only
// right at the tap that chose the file. Before, the player was never put on the page: no picture ever came.
async function sendVideo(file){
  const v=document.createElement('video');v.muted=true;v.defaultMuted=true;v.playsInline=true;
  v.setAttribute('playsinline','');v.setAttribute('webkit-playsinline','');v.setAttribute('muted','');v.preload='auto';
  v.style.cssText='position:fixed;left:0;bottom:0;width:8px;height:8px;opacity:0.01;pointer-events:none';
  document.body.appendChild(v);
  const url=URL.createObjectURL(file);v.src=url;
  try{v.load()}catch(e){}
  const un=v.play();   // right now (still inside the tap): this "unlocks" playing on iPhone; paused again at once
  if(un&&un.then)un.then(()=>{if(!v._grab)v.pause()},()=>{});
  try{return await convertClip(file,v)}finally{try{v.pause()}catch(e){}v.remove();URL.revokeObjectURL(url)}
}
async function convertClip(file,v){
  const name=safeName(file.name,'video');
  setStatus('กำลังเปิดคลิป...');
  if(!(v.readyState>=1)&&!await waitEv(v,'loadedmetadata',15000))throw new Error('เปิดคลิปไม่ได้ (ไฟล์นี้มือถือเปิดไม่ได้?)');
  const dur=v.duration;if(!(dur>0&&isFinite(dur)))throw new Error('อ่านความยาวคลิปไม่ได้');
  if(!(v.readyState>=2))await waitEv(v,'loadeddata',8000);   // the first picture is ready
  if(dur>180&&!confirm('คลิปยาว '+Math.round(dur/60)+' นาที จะใช้เวลาแปลงประมาณเท่าความยาวคลิป ทำต่อไหม?'))return;
  ...
  if(got<n){
    ...
    for(;got<n;){
      ...
      parts[got++]=lastP;
      if(got%3==0)show();
      if(fails>=8)throw new Error('แปลงคลิปนี้ไม่ได้ (มือถือไม่ยอมเปิดภาพในคลิป) ลองคลิปอื่น หรือแปลงในคอมด้วย convert_for_SomudTick.bat');
    }
  }
...
const sz=b=>b<1048576?Math.ceil(b/1024)+' KB':b<1073741824?(b/1048576).toFixed(b<104857600?1:0)+' MB':(b/1073741824).toFixed(1)+' GB';   // like the board (v11.6)
...
async function loadSd(){
  ...
  try{const d=await api('/api/sd?dir='+encodeURIComponent(cwd));SDD=d;
    ...
    $('#sdinfo').textContent=`ใช้ไป ${sz(d.used)} จาก ${sz(d.total)}`+(d.photos>=0?` · รูป ${d.many?'2000+':d.photos} · คลิป ${d.clips}`:'');
...
function renderMedia(){
  ...
  <div class="card"><p class="muted" style="margin:0 0 8px">มือถือจะย่อรูปและแปลงคลิปให้พอดีจอบอร์ดเอง ไม่ต้องใช้คอม ระหว่างแปลงคลิปอย่าปิดหรือสลับหน้า</p>
   ...
   <label class="btn dark">ส่งเกม (.gb)<input type="file" accept=".gb" multiple hidden onchange="sendGames(this.files)"></label>
   <button class="btn" onclick="loadSd()">รีเฟรช</button></div>
   <p class="muted" style="margin:8px 0 0">เกม Game Boy ไปที่โฟลเดอร์ /roms เสมอ เล่นได้ที่บอร์ด: Apps &gt; Games &gt; Game Boy (ใช้ไฟล์เกมที่เป็นของเราเองหรือแจกฟรีเท่านั้น)</p>
