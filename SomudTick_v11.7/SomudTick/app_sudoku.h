...
// v11.7: the part of a second not counted yet is kept (it was dropped at every pause: pausing often lost time)
uint32_t sdkFracMs = 0;
uint32_t sdkRunMs() { return sdkStartMs && !sg.done ? millis() - sdkStartMs : 0; }
uint32_t sdkSecs() { return sg.secs + (sdkFracMs + sdkRunMs()) / 1000; }
void sdkBank() { uint32_t t = sdkFracMs + sdkRunMs(); sg.secs += t / 1000; sdkFracMs = t % 1000; }   // add the time played to sg.secs
...
void sdkSave() {
  sdkDirty = false;
  if (!sdkHave) return;
  sdkBank(); if (sdkStartMs) sdkStartMs = millis();
...
void sdkNew(int level) {
  ...
  sg.mistakes = sg.hints = sg.done = 0; sg.secs = 0; sdkFracMs = 0;
...
void sdkPlace(int v) {   // v = 0 erase
  ...
  if (sdkSolved()) {
    sdkBank(); sg.done = 1; sdkPad = false;
...
void drawSudoku() {
  ...
  if (sdkPad && sdkSel >= 0) {
    ...
    for (int k = 0; k < 12; k++) {
      ...
      btn(x, y, bw, bh, k < 9 ? String("") : lab, on);   // the numbers are drawn once, below, in the board's plain font
      if (k < 9) txt(&fonts::FreeSans12pt7b, lab, x + bw / 2, y + bh / 2 + 1, on ? ONINK : INK, D_MC);   // (v11.5: drawn twice, 2 fonts)
...
// v11.6: the clock runs only while you play. It stops while the New game or the Solved window is open
// (it kept running there) and while the screen is off. Thinking without touching anything still counts.
void sdkClock() {
  bool run = sdkHave && !sdkNewMenu && !sg.done && pw != P_OFF;
  if (run && !sdkStartMs) sdkStartMs = millis();
  else if (!run && sdkStartMs) { sdkBank(); sdkStartMs = 0; }
}
void sudokuJoyTask() {
  sdkClock();
