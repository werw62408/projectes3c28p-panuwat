...
void drawAppIcon(int ic, int cx, int cy, uint32_t c) {
  switch (ic) {
    ...
    case IC_SWIM: {   // a small dragon (v11.6): wings up, head on the right, a curled tail
      spr.fillTriangle(cx - 3, cy, cx + 3, cy - 15, cx + 5, cy - 1, C(blend(CARD, c, 0.55f)));   // far wing
      spr.fillTriangle(cx - 8, cy + 1, cx - 15, cy - 13, cx + 1, cy - 2, C(c));                   // near wing
      for (int k = 0; k < 6; k++) spr.fillCircle(cx - 6 - k * 2.6f, cy + 5 + sinf(k * 0.9f) * 3, 3.2f - k * 0.4f, C(c));   // tail
      spr.fillCircle(cx + 2, cy + 3, 5, C(c));                                                    // body
      spr.fillCircle(cx + 10, cy - 2, 4, C(c));                                                   // head
      spr.fillTriangle(cx + 12, cy - 5, cx + 19, cy - 1, cx + 12, cy + 1, C(c));                  // snout
      spr.drawLine(cx + 8, cy - 5, cx + 4, cy - 11, C(c)); spr.drawLine(cx + 10, cy - 5, cx + 8, cy - 12, C(c));   // horns
      break; }
...
void drawGames() {
  drawAppTitle("Games");
  const char* n[N_GAMES] = {"Dragon", "Sudoku", "Sand", "Garden", "Tilt Maze", "Blocks", "Game Boy"};   // Dragon was Pixel Swim (v11.6)
  const AppIcon ic[N_GAMES] = {IC_SWIM, IC_SUDOKU, IC_SAND, IC_GARDEN, IC_MAZE, IC_BLOCKS, IC_GB};
