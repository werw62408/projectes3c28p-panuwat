...
//    joystick: push to move the blue box, press the stick = open the pad,
//              push to pick a number, press = put it in.
...
void sudokuOpen() {
  scr = S_SUDOKU;
  sdkJoy = joyDetect(); sdkJoyUsed = false;
  if (!sdkHave) sdkHave = prefs.getBytes("sdk", &sg, sizeof sg) == sizeof sg;
  if (!sdkHave) sdkNewMenu = true;
  sdkStartMs = millis();
  dirty = true;
}
void sudokuClose() { sdkSave(); sdkStartMs = 0; scr = S_GAMES; dirty = true; }
...
void sdkHint() {
  if (sdkSel >= 0 && !sg.puz[sdkSel] && sg.cur[sdkSel] != sg.sol[sdkSel] && !sg.done) {
    ...
    uint8_t before = sg.mistakes; sdkPlace(sg.sol[sdkSel]); sg.mistakes = before; sdkNotes = n; sdkSave();
  }
}
...
void sdkToolbar(bool draw, int tx, int ty) {
  if (land()) {
    ...
    if (draw) {
      ...
      txt(FS, "x" + String(sg.mistakes), sx + sw / 2, 70, sg.mistakes ? SDK_RED : SOFT, D_MC);   // x = mistakes
      ...
      txt(FS, fitText(FS, "Best " + (bst ? mmss(bst) : String("-")), sw), sx + sw / 2, 190, SOFT, D_MC);
      txt(FS, hhmm(nowT()), sx + sw / 2, 214, SOFT, D_MC);   // clock (the top bar is hidden here)
      return;
    }
  ...
  if (draw) {
    spr.fillRoundRect(4, y, 56, h, 8, C(KEYBG));
    spr.fillTriangle(12, y + 13, 19, y + 7, 19, y + 19, C(INK));
    txt(FS, "Back", 23, y + 13, INK, D_ML);
    String tm = mmss(sdkSecs());
    txt(FB, tm, 67, y + 13, INK, D_ML);
    spr.setFont(FB); int tw = spr.textWidth(tm);
    txt(FS, "x" + String(sg.mistakes), 67 + tw + 7, y + 13, sg.mistakes ? SDK_RED : SOFT, D_ML);
    btn(xHint, y, hw, h, "Hint", false);
    btn(xNew, y, nw, h, "New", sdkNewMenu);
    return;
  }
  if (!hitR(tx, ty, 0, y - 3, W, h + 5)) return;
  if (tx < 62) { sudokuClose(); return; }
...
void drawSudoku() {
  ...
  for (int i = 0; i < 81; i++) {
    ...
    if (v) {
      ...
      txt(g.cs >= 22 ? (sg.puz[i] ? FL : &fonts::FreeSans12pt7b) : (sg.puz[i] ? FB : FS), String(v), x + g.cs / 2, y + g.cs / 2 + 1, col, D_MC);
    } else if (sg.notes[i]) {
  ...
  if (sel >= 0) {   // blue frame around the chosen box
    int x = g.x0 + (sel % 9) * g.cs, y = g.y0 + (sel / 9) * g.cs;
    spr.drawRect(x, y, g.cs + 1, g.cs + 1, C(SDK_BLUE));
    spr.drawRect(x + 1, y + 1, g.cs - 1, g.cs - 1, C(SDK_BLUE));
  }
  // number pad pop-up
  if (sdkPad && sdkSel >= 0) {
    SdkPad P = sdkPadGeo();
    ...
    for (int k = 0; k < 12; k++) {
      ...
      if (k < 9) txt(FL, lab, x + bw / 2, y + bh / 2 + 1, on ? ONINK : INK, D_MC);
      if (sdkJoyUsed && k == sdkPadSel) {   // joystick cursor
        spr.drawRoundRect(x - 2, y - 2, bw + 4, bh + 4, 10, C(SDK_BLUE));
        spr.drawRoundRect(x - 3, y - 3, bw + 6, bh + 6, 11, C(SDK_BLUE));
      }
    }
  }
...
void sudokuJoyTask() {
  ...
  if (pw != P_ON) { wake(); return; }   // first touch of the stick only wakes the screen
  lastTouchMs = millis();
  sdkJoyUsed = true;
