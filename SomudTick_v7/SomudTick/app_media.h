...
bool sdOk = false;
enum FType : uint8_t { FT_DIR, FT_IMG, FT_VID, FT_TRASH };
...
const char* fsErrText(const String& e) {   // board text (the web page has its own Thai text)
  ...
  if (e == "badname") return "Use A-Z, 0-9, space, - or _";
...
void drawFiles() {
  ...
  for (size_t i = 0; i < fList.size(); i++) {
    ...
    FEntry& e = fList[i];
    bool dirLike = e.type == FT_DIR || e.type == FT_TRASH;
    spr.fillRoundRect(6, y, W - 12, FROW - 4, 8, C(CARD));
    drawFileIcon(e.type, 14, y + (FROW - 4) / 2, dirLike ? INK : SOFT);
    String right = e.type == FT_DIR ? String(">") : e.type == FT_TRASH ? String(trashN) + " >" : sizeStr(e.size);
    txt(FS, right, W - 14, y + (FROW - 4) / 2, SOFT, D_MR);
    spr.setFont(FS); int rw = spr.textWidth(right) + 12;
    txt(dirLike ? FB : FS, fitText(dirLike ? FB : FS, e.show, W - 44 - 14 - rw), 44, y + (FROW - 4) / 2, INK, D_ML);
  }
  if (fList.empty()) {
  ...
  if (fmUi == FU_CONFIRM) {
    ...
    btn(b.x + 16 + bw, b.y + b.h - 42, bw, 34, fmAsk == 3 ? "Empty" : "Delete", true, 0xD03030);
  }
...
LcdBox lcdConfirmDraw(const String& title, const String& line) {
  ...
  lcd.drawString("Delete", x + 16 + bw + bw / 2, by + 17);
...
bool lcdConfirm(const String& title, const String& line) {
  ...
  lgfx::touch_point_t tp;
  while (true) {
    while (!lcd.getTouch(&tp)) { server.handleClient(); delay(10); }
...
void filesLongPress(int x, int y) {
  ...
  if (i < 0 || i >= (int)fList.size() || fList[i].type == FT_TRASH) return;
