#pragma once
// ============================================================================
//  Internet app: Weather + PM2.5 (Khon Kaen) and short news (Thai RSS).
...
// tech news in Thai
const char* NEWS_FEEDS[] = {"https://www.blognone.com/atom.xml"};
const char* NEWS_NAMES[] = {"Blognone"};
const int N_FEEDS = 1;
...
bool wxFetch() {
  ...
  wx.temp = c["temperature_2m"] | 0.0f; wx.feels = c["apparent_temperature"] | 0.0f;
  wx.hum = c["relative_humidity_2m"] | 0; wx.rain = c["precipitation"] | 0.0f;
  wx.code = c["weather_code"] | 0; wx.wind = c["wind_speed_10m"] | 0.0f;
  for (int i = 0; i < 3; i++) {
    wx.dCode[i] = d["daily"]["weather_code"][i] | 0;
    wx.dMax[i] = d["daily"]["temperature_2m_max"][i] | 0.0f;
    wx.dMin[i] = d["daily"]["temperature_2m_min"][i] | 0.0f;
    wx.dRain[i] = d["daily"]["precipitation_probability_max"][i] | 0;
  }
  wx.ok = true; wx.at = millis();
  ...
  if (n) {
    ...
    if (!deserializeJson(a, b, n) && a["current"].is<JsonObject>()) {
      wx.pm25 = a["current"]["pm2_5"] | 0.0f; wx.pm10 = a["current"]["pm10"] | 0.0f; wx.aqi = a["current"]["us_aqi"] | 0;
      wx.aqOk = true;
    }
    free(b);
  }
...
bool newsFetch() {
  ...
  for (int f = 0; f < N_FEEDS; f++) {
    char* b; size_t n = httpGet(NEWS_FEEDS[f], &b, 400000);
    if (!n) continue;
    const char* p = b;
    while (got.size() < 30) {
      const char* it = strstr(p, "<item"); if (!it) break;
      const char* itEnd = strstr(it, "</item>"); if (!itEnd) break;
      News x; x.src = NEWS_NAMES[f]; x.t = 0;
      const char* e;
      const char* t = findTag(it, itEnd, "title", &e); if (t) x.title = cleanText(t, e);
      const char* d = findTag(it, itEnd, "description", &e); if (d) { x.desc = cleanText(d, e); if (x.desc.length() > 700) x.desc = x.desc.substring(0, 700); }
      ...
      if (x.title.length()) got.push_back(x);
      p = itEnd + 7;
    }
    free(b);
  }
...
void netLoad(bool force) {
  if (!netOnline()) { netMsg = staOn ? "No internet. Go to Settings > Choose Wi-Fi network, and join your home Wi-Fi or phone hotspot." : "Internet Wi-Fi is OFF. Turn it ON in Settings (Internet: join a Wi-Fi)."; return; }
  netMsg = "";
  // show "Loading..." before the slow download
  spr.fillRoundRect(W / 2 - 70, H / 2 - 20, 140, 40, 10, C(INK));
  txt(FB, "Loading...", W / 2, H / 2, ONINK, D_MC);
  spr.pushSprite(0, 0);
  if (netTab == 0 && (force || !wx.ok || millis() - wx.at > NET_REFRESH_MS)) { if (!wxFetch()) netMsg = "Could not get the weather. Tap Reload to try again."; }
  if (netTab == 1 && (force || news.empty() || millis() - newsAt > NET_REFRESH_MS)) {
    oilFetch();   // oil price box is optional; news still shows if it fails
    if (!newsFetch()) netMsg = "Could not get the news. Tap Reload to try again.";
  }
  lastTouchMs = millis();
  dirty = true;
}
...
void drawWeather(int top) {
  ...
  txt(FB, "C", 88 + tw, y + 32, INK, D_ML);
  txt(FB, fitText(FB, wxWord(wx.code), W - 98), 84, y + 66, INK);
  txt(FS, "Feels " + String((int)roundf(wx.feels)) + "C", 84, y + 83, SOFT);
  y += 110;
  // PM2.5
...
void oilSplit(const String& name, String& grade, String& rest) {
  ...
  while (st <= (int)name.length()) {
    ...
    else if (t.length()) { if (rest.length()) rest += ' '; rest += t; }
    st = sp + 1;
  }
}
...
OilRow oilRow(const Oil& o) {
  ...
  r.gw = 0;
  if (r.grade.length()) { forceTh = true; spr.setFont(pickFont(FB, r.grade)); r.gw = spr.textWidth(r.grade) + 5; forceTh = false; }
...
void drawNewsList(int top) {
  ...
  drawOilCard(y);
  y += oilCardH();
  if (!news.empty()) { txt(FB, "Tech news", 10, y + 2, INK); y += 26; }
  for (size_t i = 0; i < news.size(); i++) {
    auto lines = wrapText(FB, news[i].title, W - 28, 3);
    int h = newsCardH(lines.size());
    if (y + h > top - 40 && y < FTR_Y) {
      spr.fillRoundRect(6, y, W - 12, h - 4, 10, C(CARD));
      forceTh = hasThai(news[i].title);
      for (size_t k = 0; k < lines.size(); k++) txt(FB, lines[k], 14, y + 8 + k * 24, INK);
      forceTh = false;
      String meta = news[i].src + (news[i].t ? "  " + hhmm(news[i].t) : String(""));
      txt(FS, meta, 14, y + 10 + lines.size() * 24, SOFT);
    }
    y += h;
  }
...
void drawNewsStory(int top) {
  ...
  forceTh = false;
  y += 4;
  txt(FS, n.src + (n.t ? "  " + hhmm(n.t) : String("")), 12, y, SOFT); y += 26;
...
void drawNet() {
  ...
  if (netMsg.length()) {
    ...
    spr.fillRoundRect(8, top + 6, W - 16, 20 + lines.size() * 20, 10, C(CARD));
    for (size_t k = 0; k < lines.size(); k++) txt(FS, lines[k], 18, top + 16 + k * 20, INK);
    netMax = 0;
...
void netTap(int x, int y) {
  ...
  if (netTab != 1 || netMsg.length()) return;
  if (newsOpen >= 0) { newsOpen = -1; netScroll = 0; dirty = true; return; }   // tap story = back to list
  int top = netTop(), yy = top - netScroll + oilCardH() + (news.empty() ? 0 : 26);
  for (size_t i = 0; i < news.size(); i++) {
    auto lines = wrapText(FB, news[i].title, W - 28, 3);
    int h = newsCardH(lines.size());
