...
// Counted again only when the logs changed.
int unsureCnt = -1; uint32_t unsureRev = 0;
int unsureLogCount() {
  if (unsureCnt >= 0 && unsureRev == logRev) return unsureCnt;
  int n = 0;
  File root = logFs().open("/log");
  if (root) {
    for (File f = root.openNextFile(); f; f = root.openNextFile()) {
      String nm = f.name();
      if (nm.endsWith(".csv")) while (f.available()) { String l = f.readStringUntil('\n'); Ev e; if (parseLine(l, e) && !e.ok) n++; }
      f.close();
    }
    root.close();
  }
  unsureCnt = n; unsureRev = logRev;
  return n;
}
