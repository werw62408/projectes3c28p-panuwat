...
void drawUsb() {
  ...
  if (usbAsk) {
    ...
    navModal(b.x + 8 + (b.w - 24) / 4, b.y + b.h - 25);   // Back = "Not yet"
...
void usbTap(int x, int y) {
  ...
  if (y >= FTR_Y - 40 && y < FTR_Y) { usbAsk = true; dirty = true; }   // the "Leave USB drive" button
}
...
void enterUpdateMode() {
  ...
  lcd.drawString("To cancel: switch off/on", cx, cy + 40);      // (the RESET button is inside the case)
  lcd.drawString("(unplug the USB cable first)", cx, cy + 62);  // on USB power the switch can't turn it off
#ifndef SIM
  delay(800);
