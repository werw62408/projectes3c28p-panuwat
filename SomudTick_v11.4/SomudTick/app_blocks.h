...
//  Stick: left / right = move, down = faster, flick up = drop, press = turn, hold the press 1 s = pause.
...
void blocksOpen() {
  ...
  if (joyOk) joyCenter();
  blSwWas = joyOk && joyDown(); blHoldDone = blSwWas; blTouchOn = false; blUpArmed = true;
  scr = S_BLOCKS;
}
void blocksLeave() {
  if (blScore > blBest) { blBest = blScore; prefs.putUInt("blBest", (uint32_t)blBest); }
...
void blocksDraw() {
  ...
  spr.setFont(FS); spr.setTextColor(C(0x9AA7B4)); spr.setTextDatum(D_MR); spr.drawString("[Exit]", W - 6, BL_TOP / 2);
  // windows
  if (blState == BL_READY || blState == BL_PAUSE || blState == BL_OVER) {
    int ww = min(W - 24, 220), wh = blState == BL_PAUSE ? 176 : 150, wx = (W - ww) / 2, wy = (H - wh) / 2 + 6;
    ...
    if (blState == BL_READY) {
      ...
      line(FS, joyOk ? "Left / right: move   Press: turn" : "Tap left / right: move", wy + 52, SOFT);
      line(FS, joyOk ? "Down: faster   Flick up: drop" : "Tap middle: turn   Swipe down: drop", wy + 72, SOFT);
      line(FS, joyOk ? "Hold the press: pause" : "Fill a row: it goes away", wy + 92, SOFT);
      line(FB, joyOk ? "Press the stick to start" : "Tap here to start", wy + 124, INK);
...
void blTapAt(int x, int y) {
  if (y < BL_TOP + 6) { blocksLeave(); return; }   // [Exit]
...
void blocksLoop() {
  ...
  if (joyOk) {
    ...
    if (sw && !blSwWas) {
      ...
      if (blState == BL_PLAY) blTurn();   // turn at once when pressed (no waiting)
    }
    if (sw && !blHoldDone && now - blSwT > 1000 && blState == BL_PLAY) { blHoldDone = true; blState = BL_PAUSE; blMenuSel = 0; }
    ...
    if (fabsf(jx) > 0.2f || fabsf(jy) > 0.2f) lastTouchMs = now;
    if (blState == BL_PLAY) {
  ...
  } else if (OFF_MS[offIdx] && now - lastTouchMs > OFF_MS[offIdx]) { blocksLeave(); return; }
  blocksDraw();
}
