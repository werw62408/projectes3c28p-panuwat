...
//  v11.4: the pin gives its strongest current (about 40 mA instead of 20) and every setting is sent twice,
//  so a message cut by a hand / the case / distance gets a second chance. For more range: an NPN transistor
//  (S8050 / 2N2222) between the pin and the LED (see README).
...
void acLoad() {
  ...
  acIr.begin();
  gpio_set_drive_capability((gpio_num_t)PIN_IR, GPIO_DRIVE_CAP_3);   // strongest the pin can give (brighter IR light)
}
void acSave() {
...
void acSend() {
  ...
  acIr.send(1);   // the message and 1 repeat (the air con takes the same setting twice: no harm)
  acSave();
  acSentMs = millis();
...
void acUI(bool draw, int tx, int ty) {
  ...
  if (draw) {
    ...
    spr.drawCircle(W / 2 + 26, y + 15, 3, C(acS.power ? INK : SOFT)); spr.drawCircle(W / 2 + 26, y + 15, 2, C(acS.power ? INK : SOFT));   // degree sign
    txt(FB, "C", W / 2 + 36, y + 20, acS.power ? INK : SOFT, D_MC);
...
void drawAC() {
  drawAppTitle("AC Remote", millis() - acSentMs < 1200 ? String("Sent!") : String(""), false);
