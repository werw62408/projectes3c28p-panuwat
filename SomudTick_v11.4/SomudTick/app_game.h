...
void gameLoop() {
  ...
  lastFrame = millis();
  if (gs == G_PLAY) lastTouchMs = millis();   // keep the screen on while playing (not when paused or waiting)
  else if (OFF_MS[offIdx] && millis() - lastTouchMs > OFF_MS[offIdx]) {   // left on pause: back to Games, the screen can turn off
  ...
  if (b && pw == P_OFF) { wake(); b = 0; }   // screen off (paused in a pocket): a press only wakes it
