...
bool saJoy = false;
JoyBtn saBtn;   // the stick's button: short press = empty, hold 1 s = leave
...
void sandOpen() {
  ...
  saMakePalette();
  saJoy = joyOk;   // the stick found at start
  joyBtnReset(saBtn);
  saDrain = false;
  saRnd ^= esp_random() | 1;
  scr = S_SAND;
...
void sandClose() { saDrain = false; scr = S_GAMES; dirty = true; joyNavReset(); }   // (a press still held from the game does nothing in the menu)
...
void sandDraw() {
  ...
  if (saHint) {
    ...
    line(FS, saJoy ? "Tilt: wind  Hold: exit" : "No score, no rush.", by + 124, SOFT);
  }
  spr.pushSprite(0, 0);
}
...
void sandLoop() {
  ...
  int jb = saJoy ? joyBtnRead(saBtn) : 0;
  if (pw == P_OFF) { if (jb) wake(); return; }   // screen off: the sand waits (only a press of the stick wakes it)
  if (jb == 2) { sandClose(); return; }          // hold the press 1 s = back to Games
  if (jb == 1) { saDrain = !saDrain; saDrainT = millis(); saHint = false; wake(); }   // short press = empty (before the frame wait: never lost)
