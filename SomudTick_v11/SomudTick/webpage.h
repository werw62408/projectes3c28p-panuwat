...
// PIN: the board asks for it (401). The phone keeps it in a cookie, so it is asked only once.
const getPin=()=>{const m=document.cookie.match(/stpin=(\d{4})/);return m?m[1]:''};
function askPin(wrong){const p=prompt((wrong?'PIN ไม่ถูกต้อง ลองใหม่อีกครั้ง\n':'')+'ใส่ PIN 4 หลัก (ดูบนบอร์ด: Settings > Wi-Fi)');if(p===null)throw new Error('ต้องใส่ PIN ก่อนใช้งาน');document.cookie='stpin='+p.trim()+';path=/;max-age=31536000'}
