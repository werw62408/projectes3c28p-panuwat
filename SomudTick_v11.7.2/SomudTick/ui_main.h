...
void render() {
  ...
  else if (!(scr == S_SUDOKU && land())) drawHeader();   // wide Sudoku uses the full height
  if (scr == S_KEYPAD) drawKeypadFooter(); else if (scr != S_SUDOKU && scr != S_KBD && scr != S_USB && scr != S_DECK) drawFooter();   // Sudoku uses the whole screen; USB drive: stay on this page; Deck: Exit is top left
  drawRemindBar();
