...
//  - hold the stick button 1 s = back to Games (the same in every game, v11.5). [Pause] on the top bar = the pause menu
//    (Resume / Restart / Back). No stick: hold a finger where the ball should go
//  - left alone it pauses by itself; the screen dims and turns off as usual (v11.5)
...
bool mzWakeHold = false;   // the touch / press that woke the screen is still down: it does nothing
uint32_t mzRng = 1;
...
void mazeOpen() {
  ...
  mzSwWas = joyOk && joyDown(); mzHoldDone = mzSwWas; mzTouchOn = false; mzFrameT = 0; mzWakeHold = false;
  scr = S_MAZE;
}
...
void mazeDraw() {
  ...
  int bx = gameBarButtons(MZ_TOP, mzState == MZ_PLAY);   // [Pause] [Exit]
  if (b) {   // the best time, if there is room (narrow screen: it is also in the Level done window)
    String rt = "Best " + mzClock(b * 100UL);
    spr.setFont(FS);
    if ((int)spr.textWidth(rt) <= bx - 8 - (6 + lw + 8)) { spr.setTextColor(C(0x9AA7B4)); spr.setTextDatum(D_MR); spr.drawString(rt, bx - 8, MZ_TOP / 2); }
  }
...
void mazeTapAt(int x, int y) {   // a finished tap (not a hold on the board)
  if (y < MZ_TOP + 6) {   // the top bar: [Pause] [Exit]
    int h = gameBarHit(x);
    if (h == 1) { if (mzState == MZ_PLAY) mzTimeMs = millis() - mzStartMs; mazeLeave(); }
    else if (h == 2 && mzState == MZ_PLAY) { mzTimeMs = millis() - mzStartMs; mzSetState(MZ_PAUSE); }
    return;
  }
  if (mzState == MZ_READY) {
    int by = (H - 150) / 2 + 6;
...
void mazeLoop() {
  ...
  if (pw == P_OFF || mzWakeHold) {   // screen off (in a pocket): a touch or a press only wakes it, and does nothing else
    bool any = down || (joyOk && joyDown());
    if (pw == P_OFF) { if (any) { wake(); mzWakeHold = true; } return; }
    if (any) return;   // wait until it is let go
    mzWakeHold = false; mzTouchOn = false; mzSwWas = false; mzHoldDone = false; return;
  }
  ...
  // stick button: short press = start / next / choose; hold 1 s = back to Games
  if (joyOk) {
    bool sw = joyDown();
    if (sw && !mzSwWas) { mzSwT = now; mzHoldDone = false; }
    if (sw && !mzHoldDone && now - mzSwT > 1000) { mzHoldDone = true; if (mzState == MZ_PLAY) mzTimeMs = now - mzStartMs; mazeLeave(); return; }
    if (!sw && mzSwWas && !mzHoldDone && now - mzSwT > 30) {
      lastTouchMs = now;
      if (pw != P_ON) { wake(); mzSwWas = sw; return; }   // dimmed: the press only brightens the screen
    ...
    if ((mzState == MZ_PAUSE || mzState == MZ_READY) && now - moveT > 220) {
      ...
      if ((fabsf(jx) > 0.6f || fabsf(jy) > 0.6f) && pw != P_ON) { wake(); moveT = now; }   // dimmed: a push only brightens
      else if (mzState == MZ_PAUSE && fabsf(jy) > 0.6f) { mzMenuSel = constrain(mzMenuSel + (jy > 0 ? 1 : -1), 0, 2); moveT = now; lastTouchMs = now; }
      else if (mzState == MZ_READY && fabsf(jx) > 0.6f) {
  ...
  mzFrameT = now;
  if (mzState == MZ_PLAY && gameIdle()) { mzTimeMs = now - mzStartMs; mzSetState(MZ_PAUSE); }   // left alone: pause
  if (mzState == MZ_PLAY) {
    float ax = 0, ay = 0;
    ...
    if (fabsf(ax) + fabsf(ay) > 0.15f) lastTouchMs = now;   // tilting = using the board (keeps the screen on)
