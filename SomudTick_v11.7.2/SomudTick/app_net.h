#pragma once
// ============================================================================
//  Internet app: Weather + PM2.5 (Bangkok, Sathorn: RMUTK main campus, v11.6) and short news (RSS).
...
#define WX_LAT "13.71"      // Rajamangala University of Technology Krungthep, Sathorn, Bangkok (OpenStreetMap: 13.7134, 100.5375)
#define WX_LON "100.54"
#define WX_PLACE "Sathorn, BKK"
...
void pmLevel(float v, const char*& word, uint32_t& col, const char*& tip) {
  ...
  else if (v <= 37.5f) { word = "OK"; col = 0xE0B000; tip = "OK for most."; }   // (a longer text was cut on the tall screen)
...
void oilSort(std::vector<Oil>& v);   // below
...
void netStart(int tab, int cat) {
  ...
  if (xTaskCreatePinnedToCore(netTaskFn, "net", 16384, nullptr, 1, nullptr, 0) != pdPASS) {   // no memory for the download job
    netBusy = false; netHasNext = false;   // (v11.5: it used to stay "Loading..." for ever)
    netMsg = "Not enough memory to download. Restart the board (switch off and on).";
    dirty = true;
  }
}
...
// the order on screen (asked by the owner, v11.5): petrol first 98, 95, 91, E20, E85, then diesel: Hi Diesel, B20, premium diesel
int oilRank(const Oil& o) {
  String g, r; oilSplit(o.name, g, r);
  const char* P[] = {"98", "95", "91", "E20", "E85"};
  for (int i = 0; i < 5; i++) if (g == P[i]) return i;
  if (r.indexOf("Diesel") >= 0) return g == "B20" ? 6 : r.indexOf("Premium") >= 0 ? 7 : 5;
  return 8;   // anything new: at the end, in Bangchak's order
}
void oilSort(std::vector<Oil>& v) { std::stable_sort(v.begin(), v.end(), [](const Oil& a, const Oil& b) { return oilRank(a) < oilRank(b); }); }
...
void netTap(int x, int y) {
  ...
  if (y >= ty && y < ty + 30) {
    ...
    if (t >= 0 && t < 2 && t != netTab) { netTab = t; newsOpen = -1; netScroll = 0; netMsg = ""; dirty = true; render(); netLoad(false); }   // (the old tab's message is not shown on the new one)
    return;
  }
  if (netTab != 1) return;
