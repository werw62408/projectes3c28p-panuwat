...
JoyBtn gdBtn;   // the stick's button: hold 1 s = back to Games, like every game (v11.5; a short press is the menu ring's)
void gardenOpen() { gardenSync(); scr = S_GARDEN; dirty = true; joyBtnReset(gdBtn); }
void gardenJoyTask() {   // from loop()
  if (!joyOk) return;
  if (pw == P_OFF) { joyBtnReset(gdBtn); return; }   // a press that woke the screen does not count
  if (joyBtnRead(gdBtn) == 2) { scr = S_GAMES; dirty = true; joyNavReset(); }
}
...
void drawGardenScene(int todayDone, int todayGoals) {
  ...
  if (nt) {
    ...
    if (stage >= 3)
      for (int k = 0, nf = min((int)gdFruit, nt); k < nf; k++) {
        ...
        navAdd(fx - 6, fy - 6, 12, 12);   // pick it with the stick too
      }
  }
  ...
  if (stage == 0 && !raining) {   // before the first count the clock must be right (else old days can't be read yet)
    const char* msg = (timeApprox && !gdDay.length()) ? "Set the time to grow it" : "Reach a goal to grow it";
    txt(FS, fitText(FS, msg, S.w - 16), S.x + S.w / 2, gy - 34, sky.night ? 0xDDE6FF : 0x2A3A4A, D_MC);
  }
...
void drawGarden() {
  ...
  String fruit = !gdFruitPos.empty() ? String("Tap a fruit!") : "Picked " + String(gdBasket);   // fruit shows from "Small tree" on
  auto bar = [&](int by) {
